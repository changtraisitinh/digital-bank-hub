...
export default function Environments() {
    ...
    const apiDeploymentErrorCode = 902060;
    const apiDeploymentErrorStatusCode = 207;
    const intl = useIntl();
            ...
            restApi.createRevision(api.id, body)
                .then((response) => {
                    ...
                    restApi.deployRevision(api.id, response.body.id, body1)
                        .then((deployResponse) => {
                            if (deployResponse.status === apiDeploymentErrorStatusCode && deployResponse.body
                                && deployResponse.body.code === apiDeploymentErrorCode) {
                                Alert.warning(intl.formatMessage({
                                    id: 'Apis.Details.Environments.Environments.api.revision.deploy.fail',
                                    defaultMessage: 'Error while deploying API to Gateway',
                                }));
                            } else {
                                Alert.info(intl.formatMessage({
                                    id: 'Apis.Details.Environments.Environments.api.revision.deploy.success',
    ...
    function envDeploymentStatusComponent(row, allEnvDeployments) {
        ...
        if (envDetails) {
            ...
            if (total) {
                ...
                return (
                    <FormattedMessage
                        id='Apis.Details.Environments.Environments.status.no.live.gateways'
                        defaultMessage='No Active Gateways'
                    />
                );
            }
        }
