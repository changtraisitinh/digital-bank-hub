        ...
        createAPI().then((api) => {
            ...
            promisedAPIRevision.then((api1) => {
                ...
                promisedDeployment.then((res) => {
                    ...
                    if (res.status === apiDeploymentErrorStatusCode 
                            && res.body && res.body.code === apiDeploymentErrorCode) {
                        Alert.warning(intl.formatMessage({
                            id: 'Apis.Create.Default.APICreateDefault.api.revision.deploy.fail',
                            defaultMessage: 'API deployment failed for one or more gateways',
                        }));
                    } else {
                        Alert.info(intl.formatMessage({
                    ...
                    const deploymentStatus = res.body && res.body[0] ? res.body[0].status : null;
                    if (deploymentStatus === 'CREATED' ||
                        (res.status === apiDeploymentErrorStatusCode
                            && res.body && res.body.code === apiDeploymentErrorCode)) {
